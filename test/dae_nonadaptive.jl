...
sol1=solve(prob, ImplicitEuler(nlsolve=NLNewton(always_new=true, relax=0//10, max_iter=100)); adaptive=false, dt=Δt, abstol=tol, reltol=tol, initializealg=NoInit())
err1 = DiffEqToleranceTools.iteration_error(sol1)
n1 = DiffEqToleranceTools.iteration_number()

sol2=solve(prob, ImplicitEuler(nlsolve=NLNewton(always_new=false, relax=0//10, max_iter=100)); adaptive=false, dt=Δt, abstol=tol, reltol=tol, initializealg=NoInit())
err2 = DiffEqToleranceTools.iteration_error(sol2)
n2 = DiffEqToleranceTools.iteration_number()

sol3=solve(prob, ImplicitEuler(nlsolve=NLNewton(always_new=true, relax=4//10, max_iter=100)); adaptive=false, dt=Δt, abstol=tol, reltol=tol, initializealg=NoInit())
